      ...
      <aside className="hidden md:flex relative z-10 w-64 flex-col border-r border-white/5 bg-black/20 backdrop-blur-xl">
        ...
        <div className="p-4 flex flex-col gap-4">
          ...
          <div className="flex justify-center gap-6 text-xs text-slate-500 border-t border-white/5 pt-4">
            <Link href="/legal/terms" className="hover:text-indigo-400 transition-colors">Điều khoản</Link>
            <Link href="/legal/privacy" className="hover:text-indigo-400 transition-colors">Bảo mật</Link>
          </div>
        </div>
      </aside>
        ...
        <header className="absolute top-0 right-0 z-50 p-4 md:p-6 flex items-center justify-end w-full pointer-events-none">
          {showUserInfo && user ? (
            <div className="pointer-events-auto flex items-center gap-2">
              <DropdownMenu>
                <DropdownMenuTrigger className="flex items-center gap-3 outline-none bg-black/40 hover:bg-black/60 p-2 pr-4 rounded-full transition-colors cursor-pointer border border-white/10 backdrop-blur-md shadow-lg">
                  <div className="w-10 h-10 md:w-11 md:h-11 rounded-full border border-indigo-500/50 bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-[0_0_10px_rgba(99,102,241,0.5)]">
                  ...
                  <div className="flex flex-col items-start hidden sm:flex">
                    ...
                    <span className="text-xs font-medium text-indigo-400 leading-tight">
                      {user.user_metadata?.['level'] || 'A1'}
                    </span>
                  </div>
                </DropdownMenuTrigger>
